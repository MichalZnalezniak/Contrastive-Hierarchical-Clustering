...
def train(net, data_loader, train_optimizer, epoch, cfg):
    ...
    for pos_1, pos_2, target in train_bar:
        ...
        TDAG_output = TDAG(pos_1)
        pos_1, pos_2 = TDAG_output[:,0:3,:,:], TDAG_output[:,3:,:,:]
...
if __name__ == '__main__':
    ...
    TDAG = UNet(n_channels=3, n_classes=6, bilinear=False).cuda()
    ...
    optimizer = optim.Adam(list(model.parameters()) +  list(TDAG.parameters()), lr=1e-3, weight_decay=1e-6)
    print(model)
    # c = len(memory_data.classes)
    # training loop
# training loop
